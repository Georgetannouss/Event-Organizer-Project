...
const services = ref([]);//variable to hold event information
const inactiveServices = ref([]);//variable to hold event information
...
onMounted(async () => {
  try {
    ...
    inactiveServices.value = servicesResponse.filter(item => item.status === "Inactive")
...
async function submitEventUpdate() {
  ...
  try {
    const response = await updateEvent($route.params.id, event.value);
    ...
    <div class="px-10 py-20">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10 mt-10">
        ...
        <div class="flex flex-col grid-cols-3">
          <div>
            <ul v-if="services.value = !0" class="space-y-2">
              <li v-for="service in services" :key="service._id" :data-service-id="service._id"
                ...
                <label class="block w-full h-full">
                  <input type="checkbox" :id="service._id" :value="service._id"
                    :checked="event.services && event.services.includes(service._id)" v-model="event.services"
